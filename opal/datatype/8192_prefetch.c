        ...
        size_t *max_data )
{
    ...
    char *dst,
         ...
         *psrc = convertor->pBaseBuf + convertor->pStack[3].disp;
    size_t track = *max_data, iov_track;
    uint32_t i, iov_count = 0, j;
    uint32_t distance = 0;

    dst = out_iov[iov_count].iov_base;
    iov_track = out_iov[iov_count].iov_len;

    j = convertor->pStack[2].index;
    OPAL_PREFETCH( psrc + (ptrdiff_t)(iov[j].iov_base) + convertor->pStack[2].disp, 0, 3 );
    convertor->pStack[2].disp = 0;
    convertor->pStack[2].index++;

    while( convertor->pStack[2].count ){
        for( j = convertor->pStack[2].index; j < pData->iovcnt; j++ ){
            OPAL_PREFETCH( psrc + (ptrdiff_t)(iov[j].iov_base), 0, 3 );
            distance++;
            if( distance == PREFETCH_DISTANCE )
                goto end_prefetch_pack;
        }

        psrc += pData->ub - pData->lb;
        convertor->pStack[3].disp += pData->ub - pData->lb;
        convertor->pStack[2].count--;
        convertor->pStack[2].index = 0;
    }

end_prefetch_pack:
    if( convertor->pStack[2].count ){
        convertor->pStack[2].index++;
        if( convertor->pStack[2].index == pData->iovcnt ){
            psrc += pData->ub - pData->lb;
            convertor->pStack[2].index = 0;
            convertor->pStack[2].count--;
            convertor->pStack[3].disp += pData->ub - pData->lb;
        ...
        size_t *max_data )
{
    ...
    char *dst = convertor->pBaseBuf + convertor->pStack[0].disp,
         *pdst = convertor->pBaseBuf + convertor->pStack[3].disp,
         *src;
    size_t track = *max_data, iov_track;
    uint32_t i, iov_count = 0, j;
    uint32_t distance = 0;
    ...
    j = convertor->pStack[2].index;
    OPAL_PREFETCH( pdst + (ptrdiff_t)(iov[j].iov_base) + convertor->pStack[2].disp, 1, 3 );
    convertor->pStack[2].disp = 0;
    convertor->pStack[2].index++;

    while( convertor->pStack[2].count ){
        for( j = convertor->pStack[2].index; j < pData->iovcnt; j++ ){
            OPAL_PREFETCH( pdst + (ptrdiff_t)(iov[j].iov_base), 1, 3 );
            distance++;
            if( distance == PREFETCH_DISTANCE )
                goto end_prefetch_unpack;
        }

        pdst += pData->ub - pData->lb;
        convertor->pStack[3].disp += pData->ub - pData->lb;
        convertor->pStack[2].count--;
        convertor->pStack[2].index = 0;
    }
...
#define CACHE sysconf(_SC_LEVEL3_CACHE_SIZE)+sysconf(_SC_LEVEL2_CACHE_SIZE)
